...
zypper -R $target addrepo --refresh "http://download.opensuse.org/distribution/leap/15.5/repo/non-oss"         "default-repo-non-oss"
zypper -R $target addrepo --refresh "http://download.opensuse.org/distribution/leap/15.5/repo/oss"             "default-repo-oss"
zypper -R $target addrepo --refresh "http://download.opensuse.org/update/leap/15.5/oss"                        "default-repo-update-oss"
zypper -R $target addrepo --refresh "http://download.opensuse.org/update/leap/15.5/non-oss"                    "default-repo-update-non-oss"
...
zypper --non-interactive -R $target install patterns-base-base patterns-base-enhanced_base 

PARAMS=(
    kernel-default kernel-firmware-all purge-kernels-service
    grub2 grub2-i386-pc grub2-x86_64-efi

    dosfstools exfatprogs e2fsprogs cryptsetup
